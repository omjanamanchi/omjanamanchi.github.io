...
const INLINE_VIEWER_SELECTORS = 'a[target="_blank"], a[data-inline-view="true"]';
let inlineViewerElements = null;

function getInlineViewerElements() {
    if (inlineViewerElements) {
        return inlineViewerElements;
    }
    
    inlineViewerElements = {
        overlay: document.getElementById('inline-viewer-overlay'),
        window: document.querySelector('.inline-viewer-window'),
        title: document.getElementById('inline-viewer-title'),
        content: document.getElementById('inline-viewer-content'),
        message: document.getElementById('inline-viewer-message'),
        spinner: document.getElementById('inline-viewer-spinner'),
        closeButton: document.getElementById('inline-viewer-close'),
        openButton: document.getElementById('inline-viewer-open'),
        downloadButton: document.getElementById('inline-viewer-download')
    };
    
    return inlineViewerElements;
}

function setupInlineViewer() {
    const elements = getInlineViewerElements();
    const { overlay, closeButton, openButton } = elements;
    ...
    if (openButton) {
        openButton.addEventListener('click', () => closeInlineViewer());
    }
}

function closeInlineViewer() {
    const elements = getInlineViewerElements();
    const { overlay, content, message, spinner } = elements;
...
function transformYouTubeUrl(url) {
    try {
        ...
        console.warn('Unable to transform YouTube URL', error);
    }
...
function buildDrivePreviewUrl(url) {
    ...
    const docMatch = url.match(/docs\.google\.com\/(document|presentation|spreadsheets)\/d\/([^/]+)/);
...
function createEmbedIframe(src, options = {}) {
    const iframe = document.createElement('iframe');
    iframe.src = src;
    iframe.title = options.title || 'Embedded resource';
    iframe.loading = 'lazy';
...
function buildInlineEmbed(url) {
    const normalizedUrl = url.trim();
    const lowerUrl = normalizedUrl.toLowerCase();
    
    if (lowerUrl.includes('youtube.com') || lowerUrl.includes('youtu.be')) {
        const embedUrl = transformYouTubeUrl(normalizedUrl);
        return {
            element: createEmbedIframe(embedUrl, {
    ...
    if (lowerUrl.endsWith('.pdf')) {
        const pdfUrl = normalizedUrl.includes('#') ? normalizedUrl : `${normalizedUrl}#toolbar=0&navpanes=0`;
        return {
            element: createEmbedIframe(pdfUrl, {
                title: 'PDF preview'
            })
        };
    }
    ...
    if (lowerUrl.includes('drive.google.com') || lowerUrl.includes('docs.google.com')) {
        const driveUrl = buildDrivePreviewUrl(normalizedUrl);
        if (driveUrl) {
            return {
                element: createEmbedIframe(driveUrl, {
    ...
    return {
        element: createEmbedIframe(normalizedUrl, {
            title: 'Website preview'
        }),
        message: 'Some websites may block embedded previews. If the content does not load, use "Open Resource".'
    };
}

function showInlineResource(url, options = {}) {
    const elements = getInlineViewerElements();
    const { overlay, title, content, message, spinner, openButton, downloadButton } = elements;
    ...
    const resourceTitle = (options.title || '').trim() || 'Resource Preview';
    ...
    if (downloadButton) {
        if (options.forceDownload || url.toLowerCase().includes('.pdf')) {
    ...
    const embed = buildInlineEmbed(url);
    
    if (embed.message) {
        message.textContent = embed.message;
    ...
    if (embed.element) {
        embed.element.addEventListener('load', () => {
            spinner.hidden = true;
        }, { once: true });
        
        embed.element.addEventListener('error', () => {
            spinner.hidden = true;
            message.textContent = 'This content cannot be displayed here. Use "Open Resource" instead.';
            message.hidden = false;
        }, { once: true });
        ...
        spinner.hidden = true;
        message.textContent = 'Unable to display this content. Use "Open Resource" instead.';
        message.hidden = false;
    }
}

function enhanceInlineLinks(context = document) {
    if (!context) {
        return;
    }
    
    const anchors = context.querySelectorAll(INLINE_VIEWER_SELECTORS);
    
    anchors.forEach(anchor => {
        if (anchor.dataset.inlineEnhanced === 'true') {
            return;
        }
        
        anchor.dataset.inlineEnhanced = 'true';
        
        const href = anchor.getAttribute('href');
        if (!href || href.startsWith('#') || href.startsWith('mailto:') || href.startsWith('tel:') || anchor.dataset.inlineExclude === 'true') {
        ...
        anchor.addEventListener('click', (event) => {
            ...
            showInlineResource(anchor.href, {
                title: anchor.dataset.viewerTitle || anchor.getAttribute('aria-label') || anchor.textContent.trim() || 'Resource Preview'
            });
        });
    });
}
    ...
    document.querySelectorAll('.macos-dock-item').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            if (link) {
                ...
                const label = item.querySelector('.macos-dock-label');
                showInlineResource(link, {
                    title: label ? label.textContent.trim() : 'Resource Preview'
                });
            }
        });
    });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup inline viewer and enhance links
    setupInlineViewer();
    enhanceInlineLinks();
});